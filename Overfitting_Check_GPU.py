...
def train_and_test(data_folder_europe, data_folder_outside_europe, output_dir=None, iterations=35, age_span=5):
    ...
    for x in range(iterations):
        ...
        X_train_europe = torch.tensor(X_train_europe, dtype=torch.float32).to(device)
        y_train_europe = torch.tensor(y_train_europe, dtype=torch.float32).to(device)
        X_test_europe = torch.tensor(X_test_europe, dtype=torch.float32).to(device)
        y_test_europe = torch.tensor(y_test_europe, dtype=torch.float32).to(device)
        X_outside_europe = torch.tensor(X_outside_europe, dtype=torch.float32).to(device)
        y_outside_europe = torch.tensor(y_outside_europe, dtype=torch.float32).to(device)

        # Create and optimize GP regression model
        likelihood = gpytorch.likelihoods.GaussianLikelihood().to(device)
        model = GPRegressionModel(X_train_europe, y_train_europe, likelihood).to(device)
        model.train()
        likelihood.train()
        ...
        for _ in range(training_iterations):
            optimizer.zero_grad()
            output = model(X_train_europe)
            loss = -mll(output, y_train_europe)
            loss.backward()
            optimizer.step()
        ...
        with torch.no_grad(), gpytorch.settings.fast_pred_var():
            mean_predictions_europe = likelihood(model(X_test_europe)).mean.cpu().numpy()
            mean_predictions_outside_europe = likelihood(model(X_outside_europe)).mean.cpu().numpy()

        # Evaluate performance on European test set
        stats_europe = calc_stats(mean_predictions_europe, y_test_europe.cpu().numpy())
        ...
        stats_outside_europe = calc_stats(mean_predictions_outside_europe, y_outside_europe.cpu().numpy())
